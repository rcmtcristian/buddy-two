...
export const getExamplePath = (filename: string) => `src/experiments/${filename}`
...
export const isReactComponent = (param: unknown): param is React.ComponentType => {
  const isFunctionComponent = (param: unknown) => {
    return typeof param === 'function' && String(param).includes('return React.createElement')
  }
